...
public class WxQrScene extends BaseEntity
{
    ...
    /** 微信永久二维码scene值(唯一) */
    @Excel(name = "微信永久二维码scene值(唯一)")
    private Long sceneId;

    /** 分销员(系统用户ID) */
    @Excel(name = "分销员(系统用户ID)")
    private Long userId;
    ...
    /** 微信API返回ticket */
    @Excel(name = "微信API返回ticket")
    private String ticket;

    /** 二维码图片地址(本地保存) */
    @Excel(name = "二维码图片地址(本地保存)")
    ...
    /** 0分销(永久) 1活动 2通知 3公告 */
    @Excel(name = "0分销(永久) 1活动 2通知 3公告")
    private String bizType;

    /** 临时码场景字符串(act_1001等) */
    @Excel(name = "临时码场景字符串(act_1001等)")
    private String sceneStr;

    /** 过期时间(临时码30天内) */
    @JsonFormat(pattern = "yyyy-MM-dd")
    @Excel(name = "过期时间(临时码30天内)", width = 30, dateFormat = "yyyy-MM-dd")
    private Date expireTime;

    /** 落地页地址(活动/公告详情) */
    @Excel(name = "落地页地址(活动/公告详情)")
